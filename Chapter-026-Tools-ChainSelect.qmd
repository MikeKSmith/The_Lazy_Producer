# Tools - Chain Select {#Chapter-026-Tools-ChainSelect}
...
In [Tools - Variations](#Chapter-025-Tools-Variations) we looked at ways to capture different sound presets using Macro Variations. But what if you want to swap between entirely different devices, not just variations of a single one? Then you need to create a rack with multiple device chains and use different ways to select which of these chains to use. This is a handy tip for any producer or performer, Lazy or not. By having multiple devices in one Rack you can easily change between sounds using one Macro knob, or even just with different velocities or keyboard ranges.
...
If you are a very astute Lazy Producer you might imagine (or hope!) that you could specify different MIDI generators in each chain and then use automation and randomness to pick between generated sequences, like a sequential switch in a modular synth. Unfortunately, there's a limitation. Since an Instrument Rack's input is passed to all chains simultaneously, the Chain Selector only determines which chain's output is heard; it won't mute the MIDI output from unselected chains that contain their own generators.
