...
Live 12 has now introduced the concept of parameter ***modulation*** in
...
to that parameter relative to its current position. Previously, an LFO would take control of a parameter directly, overriding any manual changes. Now, it can modulate it, applying its changes relative to the parameter's current position.
...
altering the overall shape of the waveform. This is useful when you want a periodic shape but don't want the modulation to be limited to traditional waveforms. Steps essentially "bit reduces" the waveform, and introduces more
