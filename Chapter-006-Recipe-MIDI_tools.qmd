...
In this recipe we'll use the simplest possible MIDI source, a single MIDI
note, and process it with MIDI effects and routing to create an entire track from one note. 
We're going to use MIDI effects [Tools - MIDI tools](#Chapter-004-Tools-MIDI_tools)
.
And we're going to be using MIDI and audio routing which is covered in 
[Process - Routing MIDI and Audio](#Chapter-005-Process-Routing)
. We will also use modulation in sound design to keep the listener engaged - 
...
2.  Within this track, add a single MIDI clip with a single C3 note lasting one bar. Name
...
4.  I've added a Shaper to the Wavetable track and mapped its output to the Gain knob on a Utility device. The "minimum" is set
...
5.  The second Wavetable track, named “Wavetable pre-compressor,” pulls its audio from the main Wavetable track before the ducking effect ("Pre-FX"). This track's output is set to "Sends-Only," so we only hear its processed signal through the return effects, never the dry sound.
...
6.  In your tracks, you can add MIDI effects to alter the incoming chord. For example:
...
You can employ all kinds of MIDI effects here. In this example I have grouped these
effects into a MIDI Effect Rack and I'm using Macros to allow me to switch on and
change various effect parameters from a single set of 8 knobs.
...
Then we have two distinct "Note Echo" effects which are going to echo the
incoming note at 5 beats and 8 beats delay. Each effect has the "Thru" setting
...
The second "Random" effect adds more randomness to the original note, and all of
...
7.  In the screen shot for step 5, you may notice that my MIDI Note and other Generator tracks are grouped together. Let's recreate that now. Create a new MIDI track called
...
In the "Arp" track we're going to add an Arpeggiator effect (surprise!) but also
another MIDI "Note Length" effect with which we can tweak the note length of the
output from the Arpeggiator to taste, and a Velocity effect to add a
...
the sound of what is generated from the "Arpeggiator" effect - note that the
...
8.  We have mostly been looking at MIDI ***NOTE*** effects here. There’s also a much-overlooked MIDI device called “Expression Control” which allows you to map incoming MIDI data (like velocity or pitch bend) to almost any parameter in Live, including controls
...
effect controls, as well as a "random" input which will select a new value for
every MIDI input note, and "incremental" which will increment values for every
MIDI input note.
  
Also note that the effect allows you to specify Min and Max levels for each of
...
take one MIDI part and make the most of it. Using MIDI effects like this can
...
Because I'm using lots of MIDI "Note Echo" effects here, the generating input
