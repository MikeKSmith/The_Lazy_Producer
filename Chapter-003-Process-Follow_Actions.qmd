...
Ableton has had ["Follow Actions"](https://www.ableton.com/en/live-manual/11/launching-clips/#follow-actions) within Session View clips for a ***long*** time. Follow Actions allow you to perform a variety of actions at the end of a clip or after a given number of bars and beats. Follow Actions allow a performer (or a Lazy Producer) to automate the transition from one clip to another within a track or between Scenes. This allows you to determine a sequence of events - moving from one clip to another or playing the clip again, etc. This is incredibly useful for live performance where you might want certain actions to occur if left untouched, but allow the flexibility to skip to a section of a song as needed. I discussed this in a workshop presentation to Rachel K Collier's Discord community here: https://youtu.be/JhTweWbyJP8&t=25.

While Follow Actions are incredibly useful for live performance they also have incredible uses for making generative music. Follow Actions help govern the amount of variability in our generative sequences - whether there is a definite sequence of events that happen predictably, or whether there is an element of chance introduced.

::: callout-tip
## Key idea

Follow Actions allow the Ableton user to automate moving from one clip in a track to another. This automation can happen at the end of the clip or after a given number of bars and beats. In order to use Follow Actions, the clips in the track have to be contiguous - that is, no gaps between the set of clips. The Follow Actions will ***only*** apply to this set of contiguous clips - any clips outside that set of clips, even in the same track, will not be affected by the Follow Actions.

This allows the Lazy Producer (or Lazy Performer!) to set up clips that contain pre-composed riffs, rhythmic patterns, melodic lines, chords etc. and then have the Follow Action rules and probability govern what is actually played.
...
There are many different possible actions with the Follow Action. These are mostly self-explanatory but there are a few subtleties that are worth pointing out. As we point out above, Ableton needs to have contiguous groups of clips within a track to enable Follow Actions. The "First" and "Last" options above refer to ***the first and last clips within that contiguous group***, not necessarily the first and last within the track.

The "Any" option randomly jumps to any of the clips within the group of contiguous clips, including the current clip. "Other" randomly jumps to a different clip within the group. "Any" can be useful when you want to allow a repeat of the currently playing clip, while "Other" is more useful to force a jump to a different clip. "Jump" allows you to nominate a specific clip to jump to. "No Action" can be used as a "catch all" default action. With "No Action" in the alternative (right hand) Action, you can automate Ableton moving to a different clip after a certain number of bars and beats, but if this option is not taken then the clip continues playing to the end of the clip.
...
In statistics, a "[Random Walk](https://en.wikipedia.org/wiki/Random_walk "Wikipedia article on Random Walks")" is a process where the next step (forward, backward, or stationary) is determined by probabilities. With certain probability set ups, it's possible to stay in the system for a LONG period of time, taking steps.
