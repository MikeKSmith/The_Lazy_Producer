...
A key strength of Ableton's modulation devices is their flexibility: you can map a modulator to almost any parameter you can click with a mouse, even the master tempo. You can assign the parameter that is being modulated simply by click on the "Map" button in the devices and then click on a knob, switch or fader in Live. Modulation also allows you to have "hands off" performance of generative music by automating volume fades, if required.
...
```{r}
#| fig-alt: "Image shows the various parameters of a Spitfire Audio Labs plugin which is a third-party plugin."
knitr::include_graphics(path = file.path(here::here(), "images", "Labs.png"))
```

If no parameters appear, you'll likely see a “Configure” button in the device's title bar instead. When you click on this button and then open the plugin, whatever you click in the UI interface for the device or instrument will show up as a slider in the container interface (see the images below showing the Zebralette instrument as an example).
...
We'll see in [Tools - Live 12 Modulation](032-Tools-Live12_Modulation.qmd)
