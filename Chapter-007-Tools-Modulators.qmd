...
A key strength of Ableton's modulation devices is their flexibility: you can map a modulator to almost any parameter you can click with a mouse, even the tempo. You can assign the parameter that is being modulated simply by click on the "Map" button in the devices and then click on a knob, switch or fader in Live. Modulation also allows you to have "hands off" performance of generative music by automating volume fades, if required.
...
In the case of third party effects and devices you can often click on the "Down arrow" beside the device on/off button to expand the parameters that can be automated.
...
```{r}
#| fig-alt: "Image shows the various parameters of a Spitfire Audio Labs plugin which is a third-party device"
knitr::include_graphics(path = file.path(here::here(), "images", "Labs.png"))
```

If no parameters appear, you'll likely see a “Configure” button in the device's title bar instead. When you click on this button and then open the device, whatever you click in the UI interface for the device or instrument will show up as a slider in the container interface (see the images below showing the Zebralette instrument as an example).

```{r}
#| fig-alt: "Image shows the third-party Zebralette device. In order to control parameters, the user must click the Configure button"
...
```{r}
#| fig-alt: "Image shows the next step of the configure step above. Now the full user interface for the Zebralette instrument is shown."
...
We'll see in [Tools - Live 12 Modulation](#Chapter-032-Tools-Live12_Modulation)
that the LFO in Live version 12 has some additional LFO patterns and settings which get around the issue of cyclical modulation patterns.

### LFO cross-modulation

One trick from modular synthesis is to modulate LFOs with other LFOs. In the screenshot of the LFOs above, I was looking to create an LFO where there wasn't a perceptible pattern or period to its output. I took a second LFO and changed the Depth and Offset parameters of the first LFO by the second. You can extend this to include other LFOs and have them cross-modulate and feedback into each other, creating unpredictable and non-cycling behaviour. In my music, I prefer this slightly chaotic (but slow and smooth) modulation process. Below is an example where I'm using three LFOs with cross-modulation and feedback. The first LFO feeds back and modulates the third LFO Rate parameter.

```{r}
#| fig-alt: "Image shows three LFO devices. Each of the LFO devices is controlling different aspects of one of the other LFO devices. The resulting cross-modulation results in LFO modulation patterns that are not cyclical but more chaotic in nature."
knitr::include_graphics(path = file.path(here::here(), "images", "Chaotic_LFOs.png"))
```

## Envelope MIDI

The [Envelope MIDI device](https://www.ableton.com/en/live-manual/11/max-for-live-devices/#envelope-midi) is an ADSR device (Attack, Decay, Sustain, Release) that you can use to control any Live parameter. Any time the device receives a MIDI note, the envelope is triggered. This envelope can operate free of the tempo, with attack, decay, sustain and release in milliseconds, or it can be synced to the tempo. The envelope can also loop if required. The attack, decay and release rates can be linear (when set to 0%) or curved.
...
```{r}
#| fig-alt: "Image shows the Envelope MIDI effect. This device is set up to follow an Attack Decay Sustain and Release pattern modulating the filter cutoff, triggered by every incoming MIDI note event."
knitr::include_graphics(path = file.path(here::here(), "images", "MIDI_envelope.png"))
```

## Envelope Follower

The (Audio Effect) [Envelope Follower](https://www.ableton.com/en/live-manual/11/max-for-live-devices/#envelope-follower) effect is similar to the MIDI Envelope effect, but instead of being triggered by MIDI notes, it is triggered by audio. This means the Envelope Follower effect uses an incoming signal's amplitude to control other parameters.

If you adjust the "Gain" of the input until the orange envelope shows a decent range of modulation. By then tweaking the "Rise" and "Fall" parameters you can smooth out the signal if needed. There's also an option to delay the envelope rather than having it follow the input signal immediately.

Then, using the usual mapping process you can choose which parameter in Live you want to modulate using this signal. This device allows you to modulate anything using an input audio signal, whether that signal is heard in the Main audio track or not.

The devices above (LFO, MIDI Envelope and Envelope Follower) offer modular synthesis like control over pretty much any parameter in Ableton Live. This is one of the strengths of Ableton Live with Max for Live devices over the standard Ableton version - the level of control and the ability to control devices is really unparalleled. Don't forget that you can also get modulators to modulate each other... So an envelope can modulate the amplitude or rate of an LFO and an LFO can then modulate the envelope of another input. You can also get modulators to act on parameters of other tracks.

```{r}
#| fig-alt: "Image shows the Envelope follower device which is listening to incoming audio and modulating the overdrive of the DS Kick drum synth device. Parameters of the Envelope Follower are set with slow rise, fall and delay so that the effect of the modulation is delayed after the initial transient."
...
```{r}
#| fig-alt: "Image shows the Live 11 Expression Control device."
...
```{r}
#| fig-alt: "Image shows the Live 11 Expression control device similar to the previous image, but this time illustrating the various input controls that can drive the modulation."
knitr::include_graphics(path = file.path(here::here(), "images", "Expression_control_2.png"))
```

As I have said above, the combination of all of these tools are supremely powerful - so you could change envelopes with each incoming MIDI note or use that note to change an LFO setting that modulates something else. Also, be aware that the Expression Control and Envelope Follower can be used ***even if*** the MIDI or audio from the tracks are not sent to the Main audio output. OR EVEN NOT HEARD AT ALL. So you can use MIDI events to modulate items in sync with the clock regardless of tempo.

Because modulation can be mapped across tracks, you could use "dummy tracks" and clips to "listen" for MIDI and control changes which then modulate parameters and values in a completely different track.

## Strange Mod

For a long time [Dillon Baston's paid Max for Live effect "Strange Mod"](https://maxforlive.com/library/device/6872/strange-mod) was my "go to" modulation device because I wanted random but smooth modulation that wasn't cyclical. As we'll see in [Tools - Live 12 Modulation](#Chapter-032-Tools-Live12_Modulation) Ableton Live 12 now has 'Glider' and 'Stray' settings for the stock LFO modulator device which offers that random but smooth modulation. Strange Mod still has an attraction (excuse the pun) because the three modulation outputs are linked, as we'll discuss below.
...
```{r}
#| fig-alt: "Image shows Dillon Bastan's Strange Mod device which uses strange attractor chaotic processes to define the position of particles in three dimension space, which provides three lanes of modulation which can be mapped to parameters in another device."
...
```{r}
#| fig-alt: "Image shows Cong Burn's Dispatch device which allows complex combinations of cycling LFO cycles to modulate a huge range od parameters from one device. A 4 by 4 dial grid is used to dial up or down different LFOs."
...
```{r}
#| fig-alt: "Image shows Novel Music Deviate device, this time showing the Mod tab where the user can define different attributes of modulation processes to modulate parameters."
