# Process - Tuning Parameters {#Chapter-009-Process-Tuning_Parameters}

This is a key concept, so ***DON'T SKIP THIS CHAPTER***. It makes the
difference between a random collection of notes and sounds that wobble about
with very little coherence or musicality and something
that's actually listenable and identifiable as music.

::: callout-tip
## Key idea

As with the advice to [balance unexpected and predictable](#Chapter-008-Process-Balance_unexpected_and_predictable), the advice here is to
...
We defined "generative" music in [What do we mean "Generative" music?](#0000-Defintion-Generative) . We defined it as "music (that) should
...
produce sounds that ***do*** "catch the listener's ear", or worse than that -
***grate*** the listener's ear.

## The Taste Test

So, how do we avoid this? Think like a chef. Chefs constantly taste and 
adjust the seasoning as they cook to ensure the final dish is perfectly balanced.
...
Just as waves on the ocean can align to create a single, massive wave, multiple random processes can sometimes align to produce an unwelcome sonic event.
And like the waves on the ocean it's sometimes difficult to
...
The goal of all these steps is to “tune” your random processes, finding that sweet spot where everything works together to produce something you and a future listener might enjoy. As mentioned above, try to do
