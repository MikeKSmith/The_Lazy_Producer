...
are useful to transform, augment and add notes to existing MIDI sequences, but we also need to generate those sequences in the first place. Ableton Live Suite and Max for Live effects and devices offer a few options to generate sequences of notes.
...
. But to generate melodic or rhythmic sequences from scratch, you might want to use a dedicated MIDI generator like the ones discussed below. All of these devices will create MIDI note sequences - all of them can use random probability to generate note sequences that constantly change or evolve. As we have discussed in the section [Process - Balancing the unexpected and the predictable](#Chapter-008-Process-Balance_unexpected_and_predictable)
 too much randomness can be difficult for the listener to engage with. Fortunately, most of these effects allow you to tailor the amount of randomness or to slowly evolve.

::: 

In Ableton Live Suite, there are many MIDI effects that will generate note sequences and allow you to specify per note probability. Many of these were developed before Live 11 introduced per note probability in the MIDI piano roll.

## Melodic Steps

[Melodic Steps](https://www.youtube.com/watch?v=ev2GgU1D8jw) is a legacy Max for Live sequencer from the [Creative Extensions pack](https://www.ableton.com/en/packs/creative-extensions/). It is a sequencer that disentangles note pitch, octave, length, velocity and probability. This allows you to sequence these attributes separately. You can define the length of each sequence separately, so you can have all sequences with the same length for predictable MIDI sequences, or change the length of one or more to produce more elaborate patterns. Having sequences that are even numbers of steps will bring you back to repetition faster than having sequence lengths that are based on prime numbers (remember common factors from your elementary arithmetic lessons?). Note that each lane has the option to "roll the dice" and add randomness, either a little (score 2 dice) or a lot (score 5 dice). Also, note that the "Playback" setting can choose to play back at twice the speed or half the speed.

Functionality of the Melodic Steps sequencer has been largely incorporated into the newer [SQ sequencer](https://www.youtube.com/watch?v=1obXGz-Zfa8) from the [Live 12 Sequencer Pack](https://www.ableton.com/en/packs/sequencers). See 
...
["Dr Chaos"](https://www.youtube.com/watch?v=ELMM0IxNVAk) is part of the [Probability Pack by Sonic Faction](https://www.ableton.com/en/packs/probability-pack/). This MIDI effect allows you specify a sequence of notes across 8 steps. Notes can be quantised to notes in a scale using the drop down "Scale" setting on the far right at the bottom. The Network panel on the left dictates which notes follow other notes in the scale. In this example, Note 1 "C" will be followed by Note 3 "F". Note 2 "D#" will also be followed by Note 3 "F". This sequence as defined means that when press "Play" and start the transport in Ableton then the "D#" note won't necessarily play (since we start at Note 1 "C"). BUT in this case we have a little "Chaos" dialled in to the notes, which adds a little randomness, which might occasionally play the "D#". Like the "Melodic Steps" sequencer above, Dr Chaos also allows you to define potential Octave, Velocity and Length of each note ***and*** add randomness to that through the "Chaos" slider. This adds some extra "spice" to the sequence. We can also specify the "Rate" of the sequence, so you can generate faster or slower sequences.
...
[Less Concepts](https://maxforlive.com/library/device/6167/less-concepts) is a sequencer. It's easy to use, but underneath the hood is a complex world of mathematics, permutations and cellular automata rules. The good news is that you largely don't have to worry about that complexity when you use it.
...
In the image below, the key control is the large dial on the left-hand side of the [Turing Machine ](https://maxforlive.com/library/device/5988/turing-machine) device. If the dial is set to "Random" then we get a random sequence of notes in the given range (here between A2 and A5) based on the specified clock setting - in this case 8th notes. If we turn the dial completely to the right to "Lock" then we will lock in a repeating sequence of given length - here 8 notes. As we move the dial gradually back to the "Random" setting then we introduce a chance of random changes to the pattern, with the chance of randomness depending how close we are to the "Random" setting. If we turn the dial all the way to the left then we also Lock the sequence, but the repeating sequence will be twice as long i.e. it will take twice as long to come back to the start of the sequence. Because what is generated is a random sequence, we might want to apply a Scale MIDI effect to this to quantise the note outputs and conform them to a particular scale. We may also wish to use [MightM4L (MML) Probability Gate](https://maxforlive.com/library/device/6982/mm4l-probability-gate) device to "thin out" the stream of notes from the Turing Machine.
...
SEEDS also includes a very useful concept and MIDI effect that can send output to and from various devices and clocks. The "Hub" effect receives and sends MIDI information from one track to another. So it's possible to have generative effects on one track which send MIDI information to a completely different track, which can then be processed independently.

Sequences are routed to different output streams, which can then be sent out to other tracks via the Hub devices, but they can also be used internally within Polymath to use the notes of a given sequence to offset the notes in another sequence, as is shown in the image below.
...
The MDD\_\_\_\_\_SNAKE device is based on the Make Noise René modular sequencer. This sequencer by default produces 16 note values as a sequence. You can choose the note range with the slider at the top of the right hand panel and randomly choose notes from that range, or dial in your own sequence of notes using the knobs. Notes can be randomised according to scales set in the lower left corner of the device. The unique thing about this sequencer is how the sequence of notes plays using different "Snake Shapes" as chosen in the middle panel. This introduces a wide range of possible patterns to play a fixed set of notes. The pattern can be played through forwards, backwards, both forwards and back, and randomly and you can choose a number of steps to play. 

```{r}
#| fig-alt: "The image shows the MDD Snake Max for Live sequencer device in the Notes definition tab. It shows a four by four grid of dials on the right hand side corresponding to pitch. A slider at the top allows the user to define a range of pitches to randomise from. In the lower left corner the pitches can be quantised to different scales using the Fundamental value and Scale dropdowns. The middle panel defines the direction of the sequence and speed via dropdown selectors and number of steps value. Below that is a selector for choosing the 'Snake Shape' which determines the pattern of playback through the notes. In the image the pattern chosen zig-zags through the notes from bottom left to top right and back."
knitr::include_graphics(path = file.path(here::here(),"images", "MDD_Snake_Notes.png"))
```

MDD Snake also allows you to specify Gates (whether the note occurs or not) and Velocities in very similar fashion. But one very interesting feature is the ability to sequence modulation from the Custom tab. This tab allows you to specify a target using the "Map" button and then sequence values for that using the same range and randomise technique used in the Note selection or by adjusting the knobs as desired. In the image below we're sequencing the waveform of an FM operator. Note that up to three different automation or modulation sequence lanes are available.

```{r}
#| fig-alt: "The image shows the MDD Snake Max for Live sequencer device in the Custom definition tab. It shows a four by four grid of dials on the right hand side corresponding to some modulated parameter which is selected via a Map modulation selector. A slider at the top allows the user to define a range of values to randomise from. The middle panel defines the direction of the sequence and speed via dropdown selectors and number of steps value. Below that is a selector for choosing the 'Snake Shape' which determines the pattern of playback through the notes. In the image the pattern chosen goes through each row of values starting left to right in the top row then right to left in the second and so on. The image shows how the modulation sequence is changing the second Oscillator shape in an Operator instrument."
knitr::include_graphics(path = file.path(here::here(),"images", "MDD_Snake_Other.png"))
```

ELPHNT has produced a very useful YouTube video explaining MDD Snake [here](https://www.youtube.com/watch?v=v2_mdrz9XrU).

## Others I haven't mentioned

Tom Glendinning / ELPHNT is keeping a curated list of Max for Live effects he uses in his music which includes some excellent sequencers. If I haven't touched on your favourite Max for Live sequencer here, you should [check out Tom's list](https://elphnt.io/max-for-live-curated/) and review what he's presenting.
