# Tools - Random sound design with Instrument Racks {#Chapter-024-Tools-Random_Sound_Design}
...
Using the ideas presented in [Process - Tuning Parameters](#Chapter-009-Process-Tuning_Parameters) we can create Instrument Racks and Effect Racks where a single click of the Randomize button will generate new sounds, new effects that we might never think to program from scratch. Ableton Live Racks (when you Group an instrument, audio effect, MIDI effect and the Drum Rack) allow you to randomise the macro controls. You can select certain macro knobs to exclude from randomisation and through the Macro mapping control you can specify ranges of values for the macro knobs - which in turn will define the range of values in which to randomise. By carefully curating the macro ranges, you can create a nearly infinite palette of new sounds and effects, all accessible with a single click of the "Rand" button.
:::
...
You might have to iterate a few times: tweak the ranges, hit "Rand," listen to the result, and then tweak some more.
...
The techniques used by GEN and Genie aren't proprietary. I've just shown you how to do it yourself. What Tom and Misty offer, however, are expertly curated macro settings that ensure every press of the "Rand" button yields a new, usable sound.
