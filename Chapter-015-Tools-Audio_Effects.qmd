...
Creative use of audio effects can accentuate harmonics and do interesting things with those; provide some grit, noise or warble; take a rather plain sound and use modulation and effects to twist it until it forms something much more interesting and evolving. 
...
Ableton Live Suite comes with a HUGE range of audio effects and if you add in Max for Live effects there are almost too many audio-manglers out there to list or talk about in this chapter. While there are a few third-party audio effects that are worth a mention, and which I use a LOT in my productions, you can get a LONG way with just the stock Ableton Live Suite audio effects. The benefit of third party effects is where they offer something that the Live effects cannot. I'll talk about some of these below. We're going to focus here on creative effects that I use frequently in the music I make (in the ambient electronic genre) rather than mixing and mastering effects.

## Hybrid Reverb

The [Hybrid Reverb](https://www.ableton.com/en/live-manual/11/live-audio-effect-reference/#hybrid-reverb) in Live Suite combines convolution reverb with a number of digital reverb algorithms. Its Convolution Impulse Response (IR) section allows you to dial up reverb spaces from a wide variety of sources, both real and imagined; and the digital reverb section in the Algorithm section applies additional digital reverb algorithms and textures. You can blend between these or apply them in parallel and series. This in itself offers up a huge variety of sounds. This video from Ableton will help explore what can be done with the Hybrid Reverb effect <https://www.youtube.com/watch?v=yLBIOiM97Vs>.

In the Convolution IR section, you'll also find options for textural processing. This applies a textural Impulse Response (IR) to the sound, so instead of a straight reverb tail, you'll get some additional texture from the IR audio file. The other thing to point out is that there is an option to have User-defined IR. This means that you can find IR audio files for different spaces online at sites like Open Air <https://www.openair.hosted.york.ac.uk> and use some additional IR files for some famous spaces, like Hamilton Mausoleum and York Minster. 

Obviously finding the right reverb sound for ambient music is kind of a key item. You don't have to stick to just one. I often use a long, lush reverb on a return track to help smear sounds and create drone beds. I usually put this on a FX Return channel so I can route several tracks to it and save CPU. But you may also need a shorter reverb for other instruments to give them presence, in the way that a close mic to an instrument in one of these huge reverb spaces will capture more of the raw sound, while the gigantic reverb captures the instrument in the space itself.
...
Here's a third-party effect I couldn't do without. [Valhalla Supermassive](https://valhalladsp.com/shop/reverb/valhalla-supermassive/) is a free audio effect from Valhalla DSP. It's a staple of ambient and drone producers because a) it's free and b) it has some MASSIVE reverb algorithms (duh!). But what Valhalla have done rather well is to combine algorithms and delay lines so that some of its algorithms behave more like delays, while some act more like reverbs. They also have algorithms that combine both - early delay turning into massive reverb. Some have short attack, some much longer. One of my personal favourites is the "Benson Arizona" preset which will turn ANYTHING into a massive drone. Combine this with Paul Stretch, as discussed in 
[Tools - Extreme audio stretching with PaulStretch](#Chapter-016-Tools-PaulStretch)
and you'll have instant drones.

To be honest, all of Valhalla DSP's effects are excellent and reasonably priced. I particularly like the Shimmer device to add shimmer, stereo width and pitch shift reverb to sounds. Their Delay plugin provides a wide variety of delay types, including tape delay (complete with wobble and drive) but also usefully a reverse delay (where the original audio is played backwards in the delay). I like the latter as the delayed line comes back transformed compared to the input. Again this adds some interest to the part.
...
As a Lazy Producer, I prefer to let random processes tweak the Echo settings for me. I have built an audio effect rack with a trio of LFOs working on each other and then changing the feedback amount, lower and upper ranges of the EQ so that these wobble about randomly. Using an instance of Dillon Bastan's Strange Mod modulator will achieve similar effect as discussed in 
...
Saturation might seem like a strange choice for ambient music, which is often associated with "pristine" and "delicate" sounds. But there is a whole sub-genre where gritty, distorted sounds are most definitely a thing. My favourite from Live Suite is the [Saturator effect](https://www.ableton.com/en/live-manual/11/live-audio-effect-reference/#saturator) which provides a wide range of subtle distortion and drive. If you need more distortion than saturation then the [Pedal](https://www.ableton.com/en/live-manual/11/live-audio-effect-reference/#pedal) effect provides raw Overdrive, Distortion and Fuzz, Saturator allows you to drive the signal and dial in more subtle drive and overtones.
...
[Spectral Blur](https://www.ableton.com/en/packs/creative-extensions/) is from the Creative Extensions pack. It can act a little like the Freeze option from Spectral Time (see below) but to be honest, I prefer the Michael Norris Spectral Blurring and Spectral Averaging effects discussed below, and Valhalla DSP's Supermassive, as discussed above.
...
Michael Norris has created some fantastic spectral effects which can be downloaded here: <https://www.michaelnorris.info/software/soundmagic-spectral> as donation-ware.
...
The key parameter in both of these effects is the FFT size which determines the size of the sample window in which the blurring or averaging is taking place. Larger FFT sizes smear more, but take longer to fade in and out since the processing is happening over a larger window of audio. You may also find that you need to increase the gain of the output.
...
Using the output of one track as the input for another and then applying effects is a little like employing a traditional submix channel. This can be useful if you want to keep the "raw" track in the mix *and* apply effects separately, but it also allows you to pass the result of ***that*** track on into another track.
