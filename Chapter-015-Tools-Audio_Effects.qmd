...
Creative use of audio effects can accentuate harmonics and do interesting things with those; provide some grit, noise or warble; take a rather plain sound and use modulation and effects to twist it until it forms something much more interesting and evolving. All this BEFORE you even think about drowning it in reverb and delay.
...
The [Hybrid Reverb](https://www.ableton.com/en/live-manual/11/live-audio-effect-reference/#hybrid-reverb) in Live Suite combines convolution reverb with a number of digital reverb algorithms. Its Convolution Impulse Response (IR) section allows you to dial up reverb spaces from a wide variety of sources, both real and imagined; and the digital reverb section in the Algorithm section applies additional digital reverb algorithms and textures. You can blend between these or apply them in parallel and series. This in itself offers up a huge variety of sounds. This video will help explore what can be done with the Hybrid Reverb effect <https://www.youtube.com/watch?v=yLBIOiM97Vs>.
...
As a Lazy Producer, I prefer to let random processes tweak the Echo settings for me.	Minor addition for  I have built an audio effect rack with a trio of LFOs working on each other and then changing the feedback amount, lower and upper ranges of the EQ so that these wobble about randomly. Using an instance of Dillon Bastan's Strange Mod modulator will achieve similar effect as discussed in 
...
Michael Norris has created some fantastic spectral plugin effects which can be downloaded here: <https://www.michaelnorris.info/software/soundmagic-spectral> as donation-ware.
...
Using the output of one track as the input for another and then applying effects is a little like employing a traditional submix channel. This can be useful if you want to keep the "raw" track in the mix *and* apply effects separately, but it also allows you to pass the result of ***that*** track on into another track. So at any stage you can split out the audio, apply effects, route any of that signal to Return channels, then ingest the effected track into a further track for yet more processing.
