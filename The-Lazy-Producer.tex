...
specific tools and plugins you might use, and touch on some general
...
about acquiring new plugins then these are the chapters for you. If you
...
amounts of cash, but some of the plugins have an associated, although
modest, cost.
...
\begin{tcolorbox}[enhanced jigsaw, colback=white, leftrule=.75mm, toprule=.15mm, colbacktitle=quarto-callout-tip-color!10!white, opacitybacktitle=0.6, bottomtitle=1mm, breakable, colframe=quarto-callout-tip-color-frame, left=2mm, toptitle=1mm, opacityback=0, rightrule=.15mm, title=\textcolor{quarto-callout-tip-color}{\faLightbulb}\hspace{0.5em}{Key idea}, arc=.35mm, titlerule=0mm, bottomrule=.15mm, coltitle=black]
...
Note Echo} effect may at first seem a bit pointless. I mean you can take
...
Echo effects to produce nice ``multi-tap'' delays, each with different
...
effect discussed below you can easily build up evolving and interesting
...
effect} alters the pitch of incoming notes by a random amount, based on
the range you specify. In the example shown below, incoming notes will
be randomised to an increase in pitch up to 17 semi-tones above the
...
effect}\label{abletons-note-length-effect}

\href{https://www.ableton.com/en/live-manual/11/live-midi-effect-reference/\#note-length}{Note
Length} is a really useful effect to help modify the incoming MIDI and
...
effect instrument or reducing the note length can prevent CPU overload!

\includegraphics{images/Note_Length.png}

\section{Ableton's Scale effect}\label{abletons-scale-effect}

The
\href{https://www.ableton.com/en/live-manual/11/live-midi-effect-reference/\#scale}{Scale
effect} is the generative musician's friend. It takes whatever MIDI
...
\begin{tcolorbox}[enhanced jigsaw, colback=white, leftrule=.75mm, toprule=.15mm, colbacktitle=quarto-callout-tip-color!10!white, opacitybacktitle=0.6, bottomtitle=1mm, breakable, colframe=quarto-callout-tip-color-frame, left=2mm, toptitle=1mm, opacityback=0, rightrule=.15mm, title=\textcolor{quarto-callout-tip-color}{\faLightbulb}\hspace{0.5em}{Note Range}, arc=.35mm, titlerule=0mm, bottomrule=.15mm, coltitle=black]
...
\section{Ableton's Chord effect}\label{abletons-chord-effect}

Ableton's
\href{https://www.ableton.com/en/live-manual/11/live-midi-effect-reference/\#chord}{Chord
effect} adds notes to incoming MIDI notes to form chords. Up to six
...
chord over others. The Chord effect adds notes to \textbf{\emph{every}}
MIDI note it receives, and in fixed intervals above or below that
incoming MIDI note. Again, it's a good idea to place a Scale plugin
after the Chord device to ensure all the generated notes and chords stay
within your desired scale.
...
used in the Expressive Chords device in Live 12.2. Strum gives you the
ability to strum the chord with notes delayed strumming up with positive
values or strumming down with negative values. Tension will apply a
curve to the MIDI on note - making the curve faster as it goes on.
Crescendo applies note velocity ramp to the notes. In Live 11 the
Velocity of each note could be specified, but in Live 12 we can also
optionally change the Chance of each note.

Coupling the Chord effect with the Arpeggiator and Random effects is a
good way to make arpeggiated lines that do not repeat exactly. As with
most of these MIDI effects the order you specify them can provide
...
(MML) Probability Gate} is a Max for Live MIDI device that acts like a
gatekeeper. You set a probability, and it only allows that percentage of
incoming MIDI notes to pass through. This can be a very useful device to
...
Live effect from Novel Music} can add variation to incoming MIDI notes.
...
effect}\label{max-for-live-tintinnabulator-effect}

a Max for Live plugin by Milton Mermikedes,

The
\href{https://maxforlive.com/library/device/7569/tintinnabulator}{Tintinnabulator}
Max for Live effect by Milton Mermikedes implements Arvo Pärt's
...
effect}\label{max-for-live-schwartzonator-effect}

The Schwartzonator effect
...
effect}\label{max-for-live-chord-generator-effect}

The
\href{https://maxforlive.com/library/device/917/chord-generator}{Nordmann
Chord Generator} Max for Live effect is similar to the Schwartzonator
...
effect after the Chord Generator may help quantise the MIDI to sound
reasonable in the context of the scale, although selecting to use a
``b5'' chord and quantising to perfect 5th may seem counter-intuitive.
...
\textbf{\emph{audio}} utility effect
...
Max for Live effect called LFO MIDI if you need it to come before
...
When I render down the MIDI + virtual instruments, one idea is to render
without creative and mixing effects as I often need to tweak these
later, and it's problematic if I've committed to certain choices early
in the process and baked this into the resulting audio. Instead what I
have been doing is to Group the MIDI + virtual instrument track and
moving the effects to the Group level (including automation on Gain -
see below) so that the rendered audio in each track is purely MIDI +
virtual instrument. This leaves space for me to tweak the creative and
mixing effects later when I'm trying to tidy up the overall mix. You
could also alternatively route the audio from the virtual instrument
through a new track containing creative effects and leave the Group
level effects for purely mixing effects like compressor, EQ, frequency
shaping, gain etc. This would allow you to tweak the creative effects
and render down to audio at each stage - virtual instrument output, then
instrument plus effects.
...
In Ableton Live Suite, there are many plugins that will generate note
...
Melodic Steps has been largely superseded by the newer
...
Network panel on the left dictates which notes follow other notes in the
scale. In this example, Note 1 ``C'' will be followed by Note 3 ``F''.
Note 2 ``D\#'' will also be followed by Note 3 ``F''. This sequence as
defined means that when press ``Play'' and start the transport in
Ableton then the ``D\#'' note won't necessarily play (since we start at
Note 1 ``C''). BUT in this case we have a little ``Chaos'' dialled in to
the notes, which adds a little randomness, which might occasionally play
the ``D\#''. Like the ``Melodic Steps'' sequencer above, Dr Chaos also
allows you to define potential Octave, Velocity and Length of each note
\textbf{\emph{and}} add randomness to that through the ``Chaos'' slider.
This adds some extra ``spice'' to the sequence. We can also specify the
``Rate'' of the sequence, so you can generate faster or slower
sequences.
...
videos} on this plugin, where she comprehensively explores and explains
...
But if you dig into the Convolution IR section, you'll also find options
for textural processing. This applies a textural Impulse Response (IR)
to the sound, so instead of a straight reverb tail, you'll get some
additional texture from the IR audio file. The other thing to point out
is that there is an option to have User-defined IR. This means that you
can find IR audio files for different spaces online at sites like Open
Air \url{https://www.openair.hosted.york.ac.uk} and use some additional
IR files for some famous spaces, like Hamilton Mausoleum and York
Minster. Obviously finding the right reverb sound for ambient music is
kind of a key item. You don't have to stick to just one. I often use a
long, lush reverb on a return track to help smear sounds and create
drone beds. I usually put this on a FX Return channel so I can route
several tracks to it and save CPU. But you may also need a shorter
reverb for other instruments to give them presence, in the way that a
close mic to an instrument in one of these huge reverb spaces will
capture more of the raw sound, while the gigantic reverb captures the
instrument in the space itself.
...
\href{https://sonosaurus.com/paulxstretch/}{PaulXStretch} s a plugin
that stretches audio files to extreme lengths, transforming them into
evolving soundscapes and textures. This is not the type of stretching
that is available in Ableton for warping audio by a few bpm to fit a
different tempo, but instead is about sonic manipulation even to extreme
amounts.
...
\item
  ...
  process we're using another favourite Dillon Bastan plugin, the
  Strange Mod modulation device. We can then use its chaotic modulation
...
\item
  ...
  plugins. Experiment. See what works. I found that adding a Resonator
...
that were repeated by each performer. Sometimes the number of
...
range of values in which to randomise. By carefully curating the macro
ranges, you can create a nearly infinite palette of new sounds and
effects, all accessible with a single click of the ``Rand'' button.
...
this works very well. If you're using multiple scales in a single
project (for a live set, for example), it pays to be careful. Ensure
that the clips in each scene have the correct scale assigned to them to
properly trigger changes to the Current Scale. Proceed with a little
caution in live performance is my advice!
...
\begin{tcolorbox}[enhanced jigsaw, colback=white, leftrule=.75mm, toprule=.15mm, colbacktitle=quarto-callout-caution-color!10!white, opacitybacktitle=0.6, bottomtitle=1mm, breakable, colframe=quarto-callout-caution-color-frame, left=2mm, toptitle=1mm, opacityback=0, rightrule=.15mm, title=\textcolor{quarto-callout-caution-color}{\faFire}\hspace{0.5em}{Continue to use Scale Device in live performance!}, arc=.35mm, titlerule=0mm, bottomrule=.15mm, coltitle=black]
