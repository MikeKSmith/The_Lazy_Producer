...
the book you'll see how many of the tools, effects and methods we'll
...
Acquisition Syndrome (GAS). If you want to get really nerdy or think
about acquiring new devices then these are the chapters for you. If you
have already purchased Ableton Live then you have already invested a
large amount on music making equipment. My goal here is
\textbf{\emph{not}} to persuade you to part with significant additional
amounts of cash, but some of the devices have an associated, although
modest, cost.
...
Thanks to Yokermoon, kale, Subotix and owel awl for help in
...
musical ideas (notes, rhythms, timbres) for an indefinite amount of
time. In some cases the generative algorithms work \textbf{\emph{with}}
the performer, reacting to their input. In other cases the performer may
intervene with the algorithms to tweak, adjust or change direction.

\begin{tcolorbox}[enhanced jigsaw, title=\textcolor{quarto-callout-tip-color}{\faLightbulb}\hspace{0.5em}{Key idea}, toptitle=1mm, breakable, arc=.35mm, rightrule=.15mm, colbacktitle=quarto-callout-tip-color!10!white, toprule=.15mm, opacityback=0, coltitle=black, titlerule=0mm, bottomtitle=1mm, colback=white, bottomrule=.15mm, leftrule=.75mm, left=2mm, colframe=quarto-callout-tip-color-frame, opacitybacktitle=0.6]
...
\item
  ...
  have 10 bars length etc. The exact clip length isn't important; what
  matters is that they vary across tracks.
\item
...
and extra sounds only happen occasionally. Unfortunately, you can only
set the probability, not a specific condition like ``Play once, then
miss the next 3 bars'' (a feature found on some hardware sequencers,
like those from Elektron).
...
to another within a track. This allows you to determine a sequence of
events - moving from one clip to another or playing the clip again, etc.
...
take over the play position from whatever clip was previously playing
(where Follow Action was applied). This can be useful for applying
different endings to clips (the equivalent to fills in drum / rhythmic
parts). That is, you can play 3 bars of one clip, then use the fourth
bar of some other clip to provide the ending of that clip, then apply a
Follow Action to go back to the first clip, on to a different clip or
loop around to the beginning of the currently playing clip. This allows
us to set up automation that introduces musical ideas, rather than
...
double an incoming notes length, while the ``Length'' control defines
...
\begin{tcolorbox}[enhanced jigsaw, title=\textcolor{quarto-callout-tip-color}{\faLightbulb}\hspace{0.5em}{Note Range}, toptitle=1mm, breakable, arc=.35mm, rightrule=.15mm, colbacktitle=quarto-callout-tip-color!10!white, toprule=.15mm, opacityback=0, coltitle=black, titlerule=0mm, bottomtitle=1mm, colback=white, bottomrule=.15mm, leftrule=.75mm, left=2mm, colframe=quarto-callout-tip-color-frame, opacitybacktitle=0.6]
...
the predictable, or keep things the same before introducing randomness.
One important aspect of Deviate is that it has a memory, so if you like
the random pattern generated, you can ``lock'' this by moving the
``LOCK'' slider to the right. This means it acts a bit like the Turing
Machine device used in modular synthesis. Deviate has two sides - MIDI,
which we'll cover here and MOD which we'll cover in the section
...
a Max for Live MIDI effect by Milton Mermikedes,
...
\hyperref[Chapter-004-Tools-MIDI_tools]{Tools - MIDI tools} . And we're
...
\item
  Within this track, add a single MIDI clip with a single C3 note
  lasting one bar. Name this the ``Generator'' track (CTRL+R to rename).
\item
...
\tightlist
\item
  In your tracks, you can add MIDI effects to alter the incoming chord.
  For example:
\end{enumerate}
...
\tightlist
\item
  In the screen shot for step 5, you may notice that my MIDI Note and
  other Generator tracks are grouped together. Let's recreate that now.
  Create a new MIDI track called ``Note length'' and another called
  ``Arp''. In each of these tracks, set the MIDI input to Track one (the
  single note C3 track) and set Monitor to ``In''.
\end{enumerate}
...
\item
  ...
  pitch bend) to almost any parameter in Live, including controls
...
Because I'm using lots of MIDI ``Note Echo'' effects here, the
...
even the master tempo. You can assign the parameter that is being
modulated simply by click on the ``Map'' button in the devices and then
click on a knob, switch or fader in Live. Modulation also allows you to
have ``hands off'' performance of generative music by automating volume
fades, if required.
...
the plugin, whatever you click in the UI interface for the device or
...
instruments. LFOs are a key element of a modular synth rig and can be
used to modulate pretty much anything. The LFO in Ableton Live can be
...
We'll see in \href{032-Tools-Live12_Modulation.qmd}{Tools - Live 12
Modulation} that the LFO in Live version 12 has some additional LFO
...
MIDI device} an ADSR device (Attack, Decay, Sustain, Release) that you
can use to control any Live parameter. Any time the device receives a
MIDI note, the envelope is triggered. This envelope can operate free of
the tempo, with attack, decay, sustain and release in milliseconds, or
it can be synced to the tempo. The envelope can also loop if required.
The attack, decay and release rates can be linear (when set to 0\%) or
curved.
...
tracks are not sent to the Master audio output. OR EVEN NOT HEARD AT
ALL. So you can use MIDI events to modulate items in sync with the
master clock regardless of tempo.
...
\hyperref[Chapter-032-Tools-Live12_Modulation.qmd]{Tools - Live 12
...
\href{Chapter-0000-Defintion-Generative.qmd}{What do we mean
``Generative'' music?} . We defined it as ``music (that) should be able
to go on indefinitely, with enough going on to make it interesting, but
without anything particularly''sticking out'' to catch the listener's
ear''. When we use random processes to generate notes, modulate
parameters, change the timbre and qualities of sounds, there's a danger
that this randomness will produce sounds that \textbf{\emph{do}} ``catch
the listener's ear'', or more than that - \textbf{\emph{grate}} the
listener's ear.
...
In \href{Chapter-007-Tools-Modulators.qmd}{Tools - Modulators} we looked
at tools and techniques for changing parameters of Ableton and third
party devices. The act of changing something non-destructively i.e.~not
permanently is called modulation. Modulation tools can be used to
gradually change timbres, relative volumes and many other sound
attributes.
...
\href{Chapter-023-Process-Your_Unpredictable_Bandmate.qmd}{Process -
...
processes and you'll be crossing your fingers that either the result is
\textbf{\emph{the same}} as you heard previously (if Live replays the
randomness in the same way each time) or potentially different (perhaps
if you reopen the set in a later version of Live).
...
If you want to be \textbf{\emph{sure}} what chords are used, then you
should capture the output of the chord and probability process(es) by
capturing the output of the MIDI processes, as discussed above and then
use this captured MIDI track as the input to downstream processes like
the arpeggiator, pitch-shift and velocity devices.
...
A disadvantage of having effects on the Group level is that you cannot
render the Group output through ``Freeze and Flatten''. Committing the
audio by rendering allows you to free up processor power, so sometimes
it's beneficial to do this rendering process as you go. The safety net
is to retain the original tracks with devices switched off, parts and
audio output muted in case you need to return and tweak.
...
generated is a random sequence, we might want to apply a Scale MIDI
effect to this to quantise the note outputs and conform them to a
particular scale. We may also wish to use
...
In this recipe, we'll use a MIDI generator---in this case, Euclidean
Sequencer Pro---to create the initial MIDI pattern, which we'll then
feed into other tracks for further processing. We'll be using some
techniques from \hyperref[Chapter-004-Tools-MIDI_tools]{Tools - MIDI
tools} and also \hyperref[Chapter-005-Process-Routing]{Process - Routing
MIDI and Audio} .
...
\item
  ...
  patterns and because of this, by default, it only outputs four
  different notes. Since we're aiming for more melodic results, we'll
  add Random and Scale devices after it to generate a wider range of
  pitches.
\end{enumerate}
...
\item
  ...
  so that it will apply the MIDI device effects to transform the MIDI
  \textbf{\emph{before}} routing it into the new track. In this track,
  we'll add an Arpeggiator MIDI device. We can use a bell like tone for
  this track. I'm going to use Operator again, with some modulation
  applied to the second modulating oscillator (which will then vary the
  tone of the main Sine wave carrier oscillator). I'm also going to
  apply modulation to the rate of the Arpeggiator device. You can try
  out various settings here, including using an unsynced or ``Free''
  running Arpeggiator rate. You may need to tailor the amount or range
  of modulation to get Arpeggiator rates that don't swing too far
  between VERY quick and VERY slow.
\end{enumerate}
...
forms something much more interesting and evolving. All this BEFORE you
even think about drowning it in reverb and delay.
...
itself offers up a huge variety of sounds. This video will help explore
what can be done with the Hybrid Reverb effect
...
settings for me. Minor addition for I have built an audio effect rack
with a trio of LFOs working on each other and then changing the feedback
amount, lower and upper ranges of the EQ so that these wobble about
randomly. Using an instance of Dillon Bastan's Strange Mod modulator
will achieve similar effect as discussed in
\hyperref[Chapter-007-Tools-Modulators]{Tools - Modulators} While the
timing of the Echo is fixed, we have other parameters that mean that the
sound is constantly shifting in an unpredictable way. This will prevent
the Echo effect from being too ``samey'' across the track and provide
some interesting moments. It's a bit like having a pair of ``robot
hands'' that tweak the Echo settings.
...
Michael Norris has created some fantastic spectral plugin effects which
...
result of \textbf{\emph{that}} track on into another track. So at any
stage you can split out the audio, apply effects, route any of that
signal to Return channels, then ingest the effected track into a further
track for yet more processing.
...
party audio effect plugin that stretches audio files to extreme lengths,
...
standalone audio. The standalone application is often preferable: you
can audition the stretched audio in the context of your track, and once
you're happy, export the file to use in your project.
...
pentatonic scale. Many guitarists will scoff at players who riff away on
a pentatonic scale, because it is seen as being somehow a ``cheat'' and
avoids having to learn more complex scales. modes and harmonies. But for
our generative purposes, \textbf{\emph{pentatonic scales are your
absolute number 1 pick}}. You can combine any number of generative
processes working within a pentatonic scale and they will fit together
like hand in glove.
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{knitr}\SpecialCharTok{::}\FunctionTok{include\_graphics}\NormalTok{(}\AttributeTok{path =} \FunctionTok{file.path}\NormalTok{(here}\SpecialCharTok{::}\FunctionTok{here}\NormalTok{(), }\StringTok{"images"}\NormalTok{, }\StringTok{"Granulator.png"}\NormalTok{))}
\end{Highlighting}
\end{Shaded}
...
layers of effects. Be cautious when doing this, as creating a feedback
loop between return tracks can quickly result in a harsh wall of sound.
You can see that Return A (the ``Nice Reverse + Reverb'' effect) is
sending audio out to Return D (Spectral Resonator) and that that channel
is sending audio back to Return A. As we've mentioned above, using
feedback to route audio signal between Return channels is a little risky
as sound can sometimes build up quickly if channels are eventually
routed back into themselves. Adding a Limiter to the Return or Main
channels would prevent runaway sound feedback causing physical damage to
ears or speakers.
...
\href{Chapter-009-Process-Balance_unexpected_and_predictable.qmd}{Process
...
\begin{tcolorbox}[enhanced jigsaw, title=\textcolor{quarto-callout-caution-color}{\faFire}\hspace{0.5em}{Continue to use Scale Device in live performance!}, toptitle=1mm, breakable, arc=.35mm, rightrule=.15mm, colbacktitle=quarto-callout-caution-color!10!white, toprule=.15mm, opacityback=0, coltitle=black, titlerule=0mm, bottomtitle=1mm, colback=white, bottomrule=.15mm, leftrule=.75mm, left=2mm, colframe=quarto-callout-caution-color-frame, opacitybacktitle=0.6]
...
depth in
\href{Chapter-033-Tools-Live12_MIDI_Generation_Transformation.qmd}{Tools
...
In \href{Chapter-012-Tools-MIDI_Generators.qmd}{Tools - MIDI Generators}
