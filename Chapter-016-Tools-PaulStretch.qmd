...
[PaulXStretch](https://sonosaurus.com/paulxstretch/) is a third party audio effect that stretches audio files to extreme lengths, transforming them into evolving soundscapes and textures. This is not the type of stretching that is available in Ableton for warping audio by a few bpm to fit a different tempo, but instead is about sonic manipulation even to extreme amounts.
...
Stretching here means "smearing" audio, creating a blurred texture. The recognizable features of the original clip disappear, leaving behind a "ghost" of the source material. The chords and tones from the original are stretched to an extent that they turn into drones and glacially shifting harmonics.
...
The effect also features various options to tweak the output of the stretched audio, including filters, the ability enhance resonant frequencies, pitch shift and more.

## How do I use it?

You *can* use PaulXStretch as an effect on a track, but you can also use it as a standalone application. This would allow you to import, stretch, and then export the stretched file ready to import into your DAW as standalone audio. The standalone application is often preferable: you can audition the stretched audio in the context of your track, and once you're happy, export the file to use in your project.

```{r}
#| fig-alt: "The image shows the PaulXStretch effect. At the top of the device are record, rewind, play/pause, loop and freeze buttons. Below that is the main volume slider. Below that various parameters have sliders. The parameters include Stretch amount, FFT size, Number of harmonics, Frequency shift, Pitch shift, High and Low filters. A chain of buttons allow you to turn on and off various processes used in stretching and effecting the audio."
...
As an experiment, I once took the grindcore band Napalm Death’s infamous 1.3-second song, “You Suffer,” and stretched it to one minute. The result was terrifying. You can hear it here: <https://on.soundcloud.com/KXPPBRs1qdP6bSYt8>
